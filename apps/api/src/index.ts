...
import { z } from 'zod';
import { GitHubSearchError, researchQuery, searchGitHubRepositories } from './modules/research/github.js';
...
app.post('/api/projects/:id/research/github', requireAuth, asyncRoute(async (req, res) => { const project = await prisma.project.findFirst({ where: { id: String(req.params.id), userId: (req as AuthedRequest).user.sub } }); if (!project) return void fail(res, 404, 'Project not found.', 'NOT_FOUND'); const query = researchQuery(project); const run = await prisma.researchRun.create({ data: { projectId: project.id, source: 'github', query, status: 'running' } }); try { let usedFallback = false; let repositories = await searchGitHubRepositories(query); const fallbackQuery = project.domain.trim(); if (!repositories.length && fallbackQuery && fallbackQuery !== query) { usedFallback = true; repositories = await searchGitHubRepositories(fallbackQuery); } await prisma.evidenceItem.createMany({ data: repositories.map(repository => ({ researchRunId: run.id, source: 'github', ...repository })) }); const completed = await prisma.researchRun.update({ where: { id: run.id }, data: { query: usedFallback ? fallbackQuery : query, status: 'completed' }, include: { evidence: { orderBy: { createdAt: 'desc' } } } }); res.status(201).json({ run: completed, evidence: completed.evidence }); } catch (err) { await prisma.researchRun.update({ where: { id: run.id }, data: { status: 'failed' } }); throw err; } }));
app.get('/api/projects/:id/research', requireAuth, asyncRoute(async (req, res) => { const project = await prisma.project.findFirst({ where: { id: String(req.params.id), userId: (req as AuthedRequest).user.sub } }); if (!project) return void fail(res, 404, 'Project not found.', 'NOT_FOUND'); const runs = await prisma.researchRun.findMany({ where: { projectId: project.id }, orderBy: { createdAt: 'desc' }, include: { evidence: { orderBy: { createdAt: 'desc' } } } }); res.json(runs); }));
